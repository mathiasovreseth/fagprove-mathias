...
class LogRequestInterceptor {
    ...
    boolean before() {
        ...
        } else if(Environment.currentEnvironment.equals(Environment.TEST)) {
            println "URL: " +
                    "${request.scheme}://${request.serverName}" +
                    ":${request.serverPort}${request.requestURI}"

            println   "Calling controller: $controllerName " +
